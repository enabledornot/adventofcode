...
class maxGeodesCached:
    ...
    def maxGeodesU(self, resourceCount, robotCount, timeLeft):
        if timeLeft==0:
            return 0
        if timeLeft < 5 and robotCount[3] < 3:
            return 0
        if self.costs[3][0] <= robotCount[0] and self.costs[3][2] <= robotCount[2]:
            newScore = 0
            for i in range(timeLeft):
                newScore+= (robotCount[3]+i)
            return newScore
...
def getAnswer(costs):
    ...
    return mgc.maxGeodes([0,0,0], [1,0,0,0], 32)
...
if __name__=="__main__":
    # Code takes too long to finish
    # Answer gotten from finding 3 highest values from 
    # Console and multiplying them together
    sys.setrecursionlimit(50000000)
    costs = parse("input.txt")[:3]
    print(costs)
    threads = []
